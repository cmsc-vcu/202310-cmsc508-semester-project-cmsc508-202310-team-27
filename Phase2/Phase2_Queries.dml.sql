...
SELECT 
    Disease_name, CONCAT('$',Treatment_cost)
FROM 
    Disease a
LEFT JOIN 
    Treatment b
...
WHERE 
    Region_country_short_name = 'Mexico';
...
SELECT 
    CONCAT(Patient_first_name,' ',Patient_last_name) AS 'Patients', 
    Medical_Check_up_date
FROM 
    Patients_Checkups a
JOIN 
    Medical_Check_up b ON (b.Medical_Check_up_ID = a.Medical_Check_up_ID)
JOIN 
    Patient c ON (c.Patient_ID = a.Patient_ID)
ORDER BY 
     Medical_Check_up_date;
...
SELECT 
    COUNT(Patient_ethnicity) AS 'Asians', 
    Disease_name
FROM 
    Patient a
JOIN 
...
FROM 
    Patients_Checkups a
JOIN 
    Patient b ON (a.Patient_ID = b.Patient_ID)
JOIN 
    Medical_Check_up c ON (a.Medical_Check_up_ID = c.Medical_Check_up_ID)
ORDER BY  
    Patient_last_name, 
    Medical_Check_up_date;
...
FROM 
    Patients_Checkups a 
JOIN 
    Medical_Check_up b ON (a.Medical_Check_up_ID = b.Medical_Check_up_ID)
JOIN 
    Patient c ON (a.Patient_ID = c.Patient_ID)
ORDER BY 
    Patient_last_name, 
    Medical_Check_up_documentation;
...
SELECT 
    COUNT(Patient_first_name) AS "People",
...
WHERE 
    Medical_Check_up_date LIKE "2020"
GROUP BY 
    Medical_Check_up_date
ORDER BY 
    2;
...
WHERE 
    Medical_Check_up_reason LIKE '%fever%';
...
FROM 
    Patient 
LEFT JOIN 
    Region  ON (Patient_ID = Region_Patient_ID)
WHERE 
    Region LIKE "%Asia%";
...
SELECT 
    CONCAT(Patient_first_name,' ', Patient_last_name) AS "Name",
    COUNT(Patient_ethnicity) AS "Ethnicity count", 
    Patient_ethnicity, 
    Disease_name
FROM 
    Patient a
